...
if addLog:
    outputfields = ['time', 'glucose [mM]', 'glucose [mM] before temperature compensation', 'temperature [C]', 'temperature constant',
                    'theoretical temperature [C]', 'theoretical glucose [mg/dL]']
else:
...
def ref_log(timestamp):
    """
    getting reference values from the IVS log
    :param timestamp:
    :return:
    """
    ...
    try:
        with open(logPath, 'r', encoding='utf8') as f:
            ...
            for line in f:
                ...
                if 'Concentration step change to ' in line or 'Replacing liquid to ' in line:
        ...
        tdiff = int(abs((date - timestamp).total_seconds()) / 60)  # in minutes
        if tdiff > TIMEthres:  # the timestamp is too old
            ...
            concentration = float(c) if c is not None else None
            ...
            if 'Concentration step change to' in conc_theoretical:
                t_c = conc_theoretical.split('Concentration step change to ')[1].split(' ')[0]
            else:
                t_c = conc_theoretical.split(' with concentration ')[1].split('\n')[0]
            theoretical_conc = float(t_c) if t_c is not None else None
    except:
        ...
        temperature = -1
        theoretical_conc = -1
    return concentration, volume, temperature, theoretical_conc
...
class Measurement:
    ...
    def append_measurement(self):
        ...
        if m.validData:
            ...
            if addLog:
                concentration, volume, m.theoretical_temp, m.theoretical_conc = ref_log(timestamp)
                print('theoretical concentration:', m.theoretical_conc)
            else:
                m.theoretical_conc=-1
    ...
    def append_row(self, m):
        ...
        try:
            with open(self.outputfile, mode='a', newline='') as results:
                ...
                if addLog:
                    towrite['theoretical temperature [C]']= m.theoretical_temp
                    towrite['theoretical glucose [mg/dL]']= m.theoretical_conc
